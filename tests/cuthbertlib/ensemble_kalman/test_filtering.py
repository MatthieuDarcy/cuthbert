...
@pytest.mark.parametrize(("x_dim", "n_particles"), [(8, 4), (3, 10)])
def test_update_gain_association(seed, x_dim, n_particles):
    """Both associations of C_xy @ S^{-1} @ innovations match a direct reference.

    `update` associates the product whichever way is cheaper, branching on
    `n_particles` versus `x_dim`; the two branches must agree. The parametrization
    covers `n_particles < x_dim` and `n_particles >= x_dim`.
    """
    y_dim = 3
    ensemble_key, H_key, R_key, y_key = random.split(random.key(seed), 4)

    ensemble = random.normal(ensemble_key, (n_particles, x_dim))
    H = random.normal(H_key, (y_dim, x_dim))
    R_factor = random.normal(R_key, (y_dim, y_dim))
    chol_R = jnp.linalg.cholesky(R_factor @ R_factor.T + jnp.eye(y_dim))
    y = random.normal(y_key, (y_dim,))
    ...
    y_pred = ensemble @ H.T
    x_dev = ensemble - jnp.mean(ensemble, axis=0)
    y_dev = y_pred - jnp.mean(y_pred, axis=0)
    C_xy = x_dev.T @ y_dev / (n_particles - 1)
    S = y_dev.T @ y_dev / (n_particles - 1) + chol_R @ chol_R.T
    expected = ensemble + (y - y_pred) @ jnp.linalg.solve(S, C_xy.T)

    chex.assert_trees_all_close(updated, expected, rtol=1e-10, atol=1e-10)

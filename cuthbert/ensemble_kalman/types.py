...
class GetEnKFObservations(Protocol):
    """Protocol for getting observation function, observation noise lower-triangular Cholesky factor chol_R, and observation vector y for an EnKF. i.e., for state space x_t, y_t = h(x_t, model_inputs) + R_t, where R_t ~ N(0, chol_R), return (h, chol_R, y)."""

    def __call__(
        self, model_inputs: ArrayTreeLike
    ) -> tuple[ObservationFn, Array, Array]:
        """Get observation function, observation noise lower-triangular Cholesky factor chol_R, and observation vector y from model inputs.
